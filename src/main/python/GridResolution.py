...
class GridResolution:
    ...
    def solve(self):
        """ Try to solve the grid by applying each "strategy" of resolution in series until the whole grid is solved or no strategy can find new digits anymore.
            - The "Naked Single" resolution is actually the process by which each solved digit is removed from the remaining candidates in all the other cells in the same row, column and box.
               Thus it must always be performed first (to process the starting clues) and after any other strategy has found at least one new digit.
            ...
            - Each resolution has the choice to be implemented by cycling over each cell of the grid, applying a search for that cell, or by any other way.
