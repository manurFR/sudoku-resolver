...
class LockedCandidatesResolution:
    ...
    def run(self, grid):
        ...
        for xBlock in [0, 3, 6]:
            for yBlock in [0, 3, 6]:
                solvedCells += self.row_block_reduction(grid, xBlock, yBlock) + self.column_block_reduction(grid, xBlock, yBlock) + self.block_column_reduction(grid, xBlock, yBlock)
            ...
            if len(rowsWhereThisDigitIsPresentTwiceOrThrice) == 1 and len(rowsWhereThisDigitIsPresent) == 1: # if there is one and only one row with 2 or 3 cells featuring the current digit as remaining candidate...
                ...
                for i in range(SIZE):
                    if (i < xBlock or i > xBlock + 2) and d in grid.candidates[i][rowToProcess]:
                        logging.debug("Locked Candidates resolution (row_block) : removing candidate {} in ({},{}) thanks to locked candidates in block ({},{})".format(d, i, rowToProcess, xBlock, yBlock))
    ...
    def block_column_reduction(self, grid, xBlock, yBlock):
        """ For each of the 9 digits not already solved in the (xBlock, yBlock) block, check if this digit is included in the remaining candidates
             of two or three cells in a column, AND not included in the remaining candidates of the rest of the column.
            If it is the case, remove this digit from the remaining candidates of all the cells in the block outside of the column.
            Returns the number of cells solved by this process.
        """
        digits = self.remaining_candidates_in_block(grid, xBlock, yBlock)
        if not digits:
